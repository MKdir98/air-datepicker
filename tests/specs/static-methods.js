...
describe('Datepicker', function () {
    describe('getDaysCount', function () {
        it('should return 31 days in December', function () {
            assert.equal(plugin.getDaysCount(new JalaliDate(2015, 11)), 31)
        });
        it('should return 30 days in September', function () {
            assert.equal(plugin.getDaysCount(new JalaliDate(2015, 8)), 30)
        });
        it('should return 28 days in February', function () {
            assert.equal(plugin.getDaysCount(new JalaliDate(2015, 1)), 28)
        })
    });

    describe('getParsedDate', function () {
        var currentDate = new JalaliDate(),
    ...
    describe('getDecade', function () {
        it('should return array with first and last years in decade', function () {
            var decade = plugin.getDecade(new JalaliDate(2015, 1));
    ...
    describe('isSame', function () {
        var date1 = new JalaliDate(2015, 11, 14),
            date2 = new JalaliDate(2015, 11, 14),
            date3 = new JalaliDate(2015, 10, 14),
            date4 = new JalaliDate(2016, 11, 14);
    ...
    describe('less(date1, date2)', function () {
        it('should return true if date2 less then date1', function () {
            assert(plugin.less(new JalaliDate(2015, 11, 14), new JalaliDate(2015, 11, 13)))
        })
    })
    describe('bigger(date1, date2)', function () {
        it('should return true if date2 bigger then date1', function () {
            assert(plugin.bigger(new JalaliDate(2015, 11, 14), new JalaliDate(2015, 11, 15)))
        })
    })
});
