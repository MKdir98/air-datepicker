...
export function getDaysCount(date) {
    return new JalaliDate(date.getFullYear(), date.getMonth() + 1, 0).getDate();
}
...
export function subDays(date, days) {
    ...
    return new JalaliDate(year, month, _date - days);
}
...
export function copyDate(date, keepTime = true) {
    let newDate = new JalaliDate(date.getTime());
...
export function createDate(date) {
    ...
    if (!(date instanceof Date)) {
        resultDate = new JalaliDate(date);
    }
